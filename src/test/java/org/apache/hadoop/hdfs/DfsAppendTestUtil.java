...
public class DfsAppendTestUtil {
    /**
    ...
    static final Log LOG = LogFactory.getLog(DfsAppendTestUtil.class);
    ...
    static final int SMALL_BLOCK_SIZE = 1024 * 512;
    static final int SMALL_NUM_BLOCKS = 1;
    static final int SMALL_FILE_SIZE = SMALL_NUM_BLOCKS * SMALL_BLOCK_SIZE + 1;
    static long seed = -1;
    ...
    public static FileSystem createHdfsWithDifferentUsername(final Configuration conf
        ...
        return getFileSystemAs(ugi, conf);
    }
    ...
    public static FSDataOutputStream createFile(FileSystem fileSys, Path name, int repl, int blockSize)
            throws IOException {
        return fileSys.create(name, true,
                fileSys.getConf().getInt(CommonConfigurationKeys.IO_FILE_BUFFER_SIZE_KEY, 4096),
    ...
    public static void checkFullFile(FileSystem fs, Path name, int len,
        ...
        if (checkFileStatus) {
            ...
            assertEquals(len, status.getLen(),
                    "len=" + len + " but status.getLen()=" + status.getLen());
        }
